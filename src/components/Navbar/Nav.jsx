...
export default function Nav() {
  ...
  return (
    ...
    <header className="w-screen h-[5rem]  flex  justify-between items-center   overflow-hidden ">
      ...
      {/* Hamburgur Menu */}
      <button onClick={openNav} className={`text-secondary  mr-[.6rem] md:mr-[1.5rem]  lg:hidden z-51 ${open ? "hidden": "block"} `}>
         <RxHamburgerMenu  size={35}/>
      </button>

      {/* Close icon */}
      <button onClick={closeNav} className={`md:mr-[1.5rem] lg:hidden z-51 text-secondary mr-[.6rem]   ${open ? "block" : "hidden"} `}>
         <IoMdClose size={35} />
      </button>
    </header>
  );
